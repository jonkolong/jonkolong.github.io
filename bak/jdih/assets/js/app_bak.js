...
createApp({
    setup() {
        ...
        const fetchData = async () => {
            try {
                const hukumResponse = await axios.get('https://api-splp.layanan.go.id/jdih_deli_serdang/1.0/produk-hukum?tahun=semua&jenis_id=semua&tipe_id=semua&limit=10&cari=');
        ...
        const filteredData = computed(() => {
            ...
            // Jika jenis adalah 'Peraturan Bupati', proses lebih lanjut
            filtered = filtered.map(item => {
                if (item.jenis === 'Peraturan Bupati') {
                    return { ...item, teuBadan: item.teuBadan.replace(/Kabupaten\s*/i, '') }; // Proses sesuai dengan aturan
                }
                return item;
            });

            const start = (currentPage.value - 1) * itemsPerPage.value;
            return filtered.slice(start, start + itemsPerPage.value); // Membatasi hasil pencarian per halaman
        });
        ...
        const totalPages = computed(() => {
            ...
            return Math.ceil(filtered.length / itemsPerPage.value); // Hitung total halaman berdasarkan data yang terfilter
        });
